...
public class MallRunner {
	public static void main(String[] args) {
		...
		// 1. Sort all malls by ID descending order
		System.out.println("********sort*********");
		mall.stream().sorted((dto1, dto2) -> Double.compare(dto2.getId(), dto1.getId())).collect(Collectors.toList())
				.forEach(dto -> System.out.println(dto));

		// 2. Sort all malls by name desc order
		System.out.println("********sort*********");
		mall.stream().sorted((dto1, dto2) -> dto2.compareTo(dto1)).collect(Collectors.toList())
				.forEach(dto -> System.out.println(dto));
		...
		System.out.println("*****************");
		Collection<MallDTO> landDimension = mall.stream().filter(dto -> dto.getLandDimension() < 100000)
				.collect(Collectors.toList());
		System.out.println("Malls with landDimension less than 1 lakh: " + landDimension);

		// 5. Collect only the names of the malls
		System.out.println("********sort*********");
		mall.stream().map((dto) -> dto.getName()).collect(Collectors.toList()).forEach(dto -> System.out.println(dto));

		// 6. Collect only landDimension and sort it in ascending order
		System.out.println("*****************");
		Collection<Double> landDimensions = mall.stream().map(MallDTO::getLandDimension).sorted()
				.collect(Collectors.toList());
